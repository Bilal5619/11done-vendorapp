...
export default function ProfileScreen() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      setSelectedCategoryIds(
        getSelectedCategoryIds(nextVendor).filter((id) =>
          activeCategoryIds.has(String(id)),
        ),
      );
      setForm({
        ...
        postcode:
          nextVendor.profile?.postcode ?? nextVendor.profile?.zip_code ?? "",
      });
    } catch (loadError) {
      ...
      if (storedVendor) {
        ...
        setForm({
          ...
          postcode:
            storedVendor.profile?.postcode ??
            storedVendor.profile?.zip_code ??
            "",
        });
      }
    } finally {
  ...
  async function saveProfile() {
    ...
    try {
      const activeCategoryIds = new Set(
        availableCategories.map((category) => String(category.id)),
      );

      const validCategoryIds = selectedCategoryIds
        .filter((id) => activeCategoryIds.has(String(id)))
        .map(Number);

      const response = await updateProfile({
        ...form,
        service_category_ids: validCategoryIds,
      });
      const nextVendor = getVendorFromResponse(response);
      setVendor(nextVendor);
      setSelectedCategoryIds(getSelectedCategoryIds(nextVendor));
      setSuccess(
        "Profile updated successfully. Your selected service categories are now active.",
      );
    } catch (saveError) {
  ...
  async function savePhoto(uri: string) {
    ...
    try {
      ...
      setSuccess(
        "Company logo updated. It will be printed on your certificates.",
      );
    } catch (uploadError) {
  ...
  async function takeLogoPhoto() {
    ...
    if (!permission.granted) {
      Alert.alert(
        ...
        "Please allow camera access to capture your company logo.",
      );
      return;
    }
  ...
  async function chooseLogoPhoto() {
    ...
    if (!permission.granted) {
      Alert.alert(
        ...
        "Please allow photo access to select your company logo.",
      );
      return;
    }
  ...
  async function removeLogoPhoto() {
    ...
    try {
      ...
      setError(
        normalizeApiError(removeError, "Unable to remove the logo.").message,
      );
    } finally {
      setIsUploadingPhoto(false);
    }
  ...
  return (
    ...
    >
      ...
      <SectionIntro
        kicker="Vendor Profile"
        title={form.name || vendor?.username || "Profile"}
        text="Manage your contact, business, and account details in one place."
      />
      ...
      >
        <Card>
          ...
          <Text style={ui.muted}>
            Your trading name, address, VAT status and bank details — used on
      ...
      <Card style={focusSection === "logo" ? styles.focusCard : undefined}>
        <View
          onLayout={(event: LayoutChangeEvent) => {
            sectionPositions.current.logo = event.nativeEvent.layout.y;
          }}
        >
          <Text style={ui.cardTitle}>Company logo</Text>
          <Text style={ui.muted}>
            Upload your company logo. It is printed in the header of every
            certificate you generate, next to the Gas Safe logo.
          </Text>
          ...
          <View style={ui.wrapRow}>
            <Pressable
              ...
              style={[ui.secondaryButton, styles.logoButton]}
            >
              <Text style={ui.secondaryButtonText}>
                {logoUri ? "Retake photo" : "Take photo"}
              </Text>
            </Pressable>
            <Pressable
              disabled={isUploadingPhoto}
              onPress={chooseLogoPhoto}
              style={[ui.secondaryButton, styles.logoButton]}
            >
              <Text style={ui.secondaryButtonText}>
                {logoUri ? "Choose another" : "Choose photo"}
              </Text>
            </Pressable>
            {logoUri ? (
              <Pressable
                disabled={isUploadingPhoto}
                onPress={removeLogoPhoto}
                style={[ui.secondaryButton, styles.logoButton]}
              >
                <Text style={[ui.secondaryButtonText, { color: "#ff8585" }]}>
                  Remove
                </Text>
              </Pressable>
            ) : null}
          ...
          {isUploadingPhoto ? (
            <ActivityIndicator color="#ff6a00" style={{ marginTop: 10 }} />
          ) : null}
        </View>
      </Card>
      ...
      <Card style={focusSection === "profile" ? styles.focusCard : undefined}>
        ...
        <View
          onLayout={(event: LayoutChangeEvent) => {
            sectionPositions.current.profile = event.nativeEvent.layout.y;
          }}
        >
          <View
            onLayout={(event: LayoutChangeEvent) => {
              sectionPositions.current.serviceCategory =
          ...
          >
            <View style={styles.categoryField}>
              ...
              <Text style={ui.muted}>
                Select every trade you currently provide. You can return here
                and add more later.
              </Text>
                    ...
                    >
                      <View
                        style={[
                          ...
                          selected && styles.categoryCheckSelected,
                        ]}
                      >
                        <Text
                          style={[
                            styles.categoryCheckText,
                            selected && styles.categoryCheckTextSelected,
                          ]}
                        >
                          {selected ? "✓" : "+"}
                        </Text>
        ...
        <View
          onLayout={(event: LayoutChangeEvent) => {
            sectionPositions.current.postcode = event.nativeEvent.layout.y;
          }}
        >
          <Input
            label="Postcode"
      ...
      <Card>
        ...
        <Input
          ...
          secure
          onChangeText={(value) =>
            setPasswords((current) => ({ ...current, current_password: value }))
          }
        />
        <Input
          label="New password"
      ...
      <Pressable onPress={handleLogout} style={ui.secondaryButton}>
        <Text style={[ui.secondaryButtonText, { color: "#ff8f4a" }]}>
          Logout
        </Text>
      </Pressable>
    </ProtectedScreen>
