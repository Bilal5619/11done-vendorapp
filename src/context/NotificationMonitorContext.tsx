...
const NotificationMonitorContext =
  createContext<NotificationMonitorValue | null>(null);
...
export function NotificationMonitorProvider({ children }: PropsWithChildren) {
      ...
      try {
        ...
        const unreadNotifications =
          notificationsResult.status === "fulfilled"
            ? notificationsResult.value.notifications.filter(
        ...
        const matchingNotifications = unreadNotifications.filter(
          (notification) =>
            INCOMING_JOB_TYPES.has(
              normalizeNotificationType(notification.type),
            ),
        );

        const notificationCandidate = matchingNotifications.find(
          (notification) => !isHandled(notification, handledKeysRef.current),
        );
        ...
        if (assignedJobs) {
          ...
          if (knownIds === null) {
            ...
            const newJob = assignedJobs.find(
              (job) => !knownIds.has(String(job.id)),
            );
        ...
        if (
          fallbackJob ||
          candidate.related_id === undefined ||
          candidate.related_id === null
        )
          return;
        ...
        try {
          ...
          const responseJob =
            candidateType === "new_job_available" || candidateType === "new_job"
              ? ((await getAvailableJobs()).find(
                  (job) => String(job.id) === String(candidate.related_id),
                ) ?? null)
              : extractJob(await getJob(candidate.related_id));
          if (
            active &&
            incomingRef.current &&
            getNotificationKey(incomingRef.current) ===
              getNotificationKey(candidate)
          ) {
  ...
  const rememberHandled = useCallback(
    async (notification: VendorNotification) => {
  ...
  const handleIncomingAction = useCallback(
    async (action: IncomingJobAction) => {
      const notification = incomingRef.current;
      if (!notification || isActioning) return;

      setActionError("");
      setIsActioning(true);
      const jobId = incomingJob?.id ?? notification.related_id;

      if (action === "view" || action === "dismiss") {
        await Promise.all([
          rememberHandled(notification),
          markReadSafely(notification),
        ]);
        closeIncoming();
      ...
      if (jobId === undefined || jobId === null) {
        setActionError(
          "This notification does not include a job ID. Please open the Jobs screen.",
        );
        setIsActioning(false);
        return;
      }
      ...
      try {
        ...
        setActionError(
          normalizeApiError(error, `Could not ${action} this job.`).message,
        );
        setIsActioning(false);
      }
    },
...
export function useNotificationMonitor() {
  ...
  if (!context)
    throw new Error(
      "useNotificationMonitor must be used inside NotificationMonitorProvider",
    );
  return context;
}
...
function getNotificationKey(notification: VendorNotification) {
  return String(
    notification.id ??
      `${notification.type ?? "job"}:${notification.related_id ?? "unknown"}`,
  );
}

function getJobHandledKey(notification: VendorNotification) {
  return notification.related_id === undefined ||
...
function isHandled(notification: VendorNotification, handledKeys: Set<string>) {
  const jobKey = getJobHandledKey(notification);
  return (
    handledKeys.has(getNotificationKey(notification)) ||
...
function extractJob(
  response: { appointment?: JobDetail; job?: JobDetail } | JobDetail,
): JobDetail | null {
